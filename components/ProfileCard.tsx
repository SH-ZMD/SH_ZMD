    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center md:items-end justify-between mt-6 md:mt-8 gap-5 md:gap-6 relative z-10">
        ...
        <div className="flex gap-2 md:gap-3 flex-wrap justify-center md:justify-end w-full md:w-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <SocialBtn type="email" url={siteConfig.social?.email} />
...
function SocialBtn({ type, url, onClick }: { type: string, url?: string, onClick?: () => void }) {
  ...
  const isPlaceholder = !cleanUrl || ['暂无', '暂未透露', '无', '-'].includes(cleanUrl);
  const safeUrl = isPlaceholder
    ? undefined
    : /^https:\/\//i.test(cleanUrl)
      ? cleanUrl
      : type === 'email' && /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(cleanUrl)
        ? `mailto:${cleanUrl}`
        : undefined;
  const getIcon = () => {
    switch (type) {
  ...
  if (!safeUrl && !onClick) return null;
  return safeUrl ? <a href={safeUrl} target={safeUrl.startsWith('mailto:') ? undefined : '_blank'} rel="noopener noreferrer" aria-label={type}>{content}</a> : content;
}
