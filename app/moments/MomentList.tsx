...
import { Eye, EyeOff, MapPin, MessageSquare, Clock, Sparkles, Search, ArrowDownAZ, ArrowUpZA, ChevronLeft, ChevronRight, Ghost, Plus, Image as ImageIcon, X, Send, Link as LinkIcon, Zap, Trash2, AlertTriangle } from 'lucide-react';
...
export default function MomentList({ moments, authorName, avatarUrl }: any) {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (!file.type.startsWith('image/')) {
          showToast(`第 ${i + 1} 个文件不是图片，已跳过`, "warning");
          continue;
        }
        ...
        else showToast(data.message || `第 ${i + 1} 张图片导入失败`, "error");
      }
      if (newUrls.length > 0) {
      ...
      showToast(`本地图片导入失败: ${error.message}`, "error");
    } finally {
  ...
  const toggleHidden = async (moment: any) => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${config.api_port}/api/content-visibility/set`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'moment', id: moment.id, hidden: nextHidden })
      });
      const data = await res.json();
      if (data.success) {
  ...
  const renderMomentCard = (moment: any) => (
    <motion.div
      ...
      className={`flex flex-col bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl rounded-[40px] shadow-xl border border-white/40 dark:border-white/10 p-8 md:p-10 transition-shadow hover:shadow-2xl overflow-hidden relative group w-full ${moment.hidden ? 'opacity-60' : ''}`}
    >
      {/* 霓虹队列状态提示 */}
      {moment.isPending && (
              ...
              <div className="flex flex-col gap-4 mb-6">
                ...
                >
                  ...
                  <span className="text-sm font-bold text-slate-500 dark:text-slate-400">
                    {isUploading ? '正在导入本地图片...' : '点击或拖拽本地图片到这里'}
                  </span>
                </div>
                  ...
                  {newMoment.images.map((img, idx) => (
                    <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm group">
                      ...
                      <button
                        ...
                        className="absolute top-1.5 right-1.5 w-6 h-6 flex items-center justify-center bg-black/60 text-white rounded-full opacity-30 group-hover:opacity-100 transition-opacity hover:bg-red-500"
                      >
                        <X size={14} />
                      </button>
                    </div>
