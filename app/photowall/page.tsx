...
export default function PhotoWallPage() {
  ...
  const uploadLocalImage = async (file: File) => {
    ...
    if (!data.success || !data.url) throw new Error(data.message || '图片导入失败');
  ...
  const handleAlbumCoverFile = async (file?: File) => {
    if (!file) return;
    try {
      showToast('正在导入相册封面...', 'info');
      const url = await uploadLocalImage(file);
      setAlbumModal(prev => ({ ...prev, data: { ...prev.data, cover: url } }));
      showToast('封面已导入', 'success');
    } catch (error: unknown) {
      showToast(error instanceof Error ? error.message : '封面导入失败', 'error');
    } finally {
  ...
  const handlePhotoFiles = async (files?: FileList | null) => {
    ...
    try {
      showToast(`正在导入 ${files.length} 张照片...`, 'info');
      const urls: string[] = [];
      for (const file of Array.from(files)) {
        urls.push(await uploadLocalImage(file));
      }

      if (urls.length === 1) {
        setPhotoModal(prev => ({ ...prev, data: { ...prev.data, url: urls[0] } }));
        showToast('照片已导入', 'success');
        return;
      }
      ...
      album.photos = [...urls.map(url => ({ url, caption: '' })), ...album.photos];
      const next = [...editableAlbums];
      setEditableAlbums(next);
      setCurrentAlbum({ ...album });
      syncToQueue(next);
      setPhotoModal({ ...photoModal, isOpen: false });
      showToast('多张照片已加入相册', 'success');
    } catch (error: unknown) {
      showToast(error instanceof Error ? error.message : '照片导入失败', 'error');
    } finally {
      if (photoFilesInputRef.current) photoFilesInputRef.current.value = '';
    }
  };
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative w-full max-w-md bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[40px] border border-white/20 p-8 shadow-2xl">
              ...
              <div className="space-y-5">
                ...
                <input ref={albumCoverInputRef} type="file" accept="image/*" onChange={e => handleAlbumCoverFile(e.target.files?.[0])} className="hidden" />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="py-3 rounded-2xl bg-indigo-500 text-white text-xs font-black shadow-lg shadow-indigo-500/30 hover:bg-indigo-600 transition-all active:scale-95"
                  >
                    本地导入封面
                  </button>
                  <button
                    type="button"
                    onClick={() => { setImgToolTarget('album'); setIsImgToolOpen(true); }}
                    className="py-3 rounded-2xl bg-slate-800 text-white text-xs font-black hover:bg-slate-700 transition-all active:scale-95"
                  >
                    外链/图床
                  </button>
                </div>
        ...
        {photoModal.isOpen && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative w-full max-w-md bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[40px] border border-white/20 p-8 shadow-2xl">
              ...
              <div className="space-y-5">
                <input ref={photoFilesInputRef} type="file" accept="image/*" multiple onChange={e => handlePhotoFiles(e.target.files)} className="hidden" />
                {photoModal.mode === 'add' && (
                  <>
                    <div className="relative group">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        type="button"
                        onClick={() => photoFilesInputRef.current?.click()}
                        className="py-3 rounded-2xl bg-indigo-500 text-white text-xs font-black shadow-lg shadow-indigo-500/30 hover:bg-indigo-600 transition-all active:scale-95"
                      >
                        本地导入照片
                      </button>
                      <button
                        type="button"
                        onClick={() => { setImgToolTarget('photo'); setIsImgToolOpen(true); }}
                        className="py-3 rounded-2xl bg-slate-800 text-white text-xs font-black hover:bg-slate-700 transition-all active:scale-95"
                      >
                        外链/图床
                      </button>
