...
export default function ChatterBoard({ chatters: initialChatters }: { chatters: Chatter[] }) {
  ...
  const allTags = useMemo(() => {
    ...
    const visibleChatters = canManage ? chatters : chatters.filter(c => !c.hidden);
    visibleChatters.forEach(c => c.tags?.forEach(t => tags.add(t)));
    return ["全部", ...Array.from(tags)];
  }, [chatters, canManage]);

  const filteredChatters = useMemo(() => {
    const query = searchQuery.trim().toLowerCase();
    const visibleChatters = canManage ? chatters : chatters.filter(chatter => !chatter.hidden);
    return visibleChatters.filter(chatter => {
  ...
  }, [chatters, searchQuery, activeTag, canManage]);

  const toggleHidden = async (chatter: Chatter) => {
    if (!canManage) return;
    const nextHidden = !chatter.hidden;
    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      const config = await configRes.json();
      const res = await fetch(`http://127.0.0.1:${config.api_port}/api/content-visibility/set`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'chatter', id: chatter.slug, hidden: nextHidden })
      });
      ...
      showToast('无法连接到 Python 后台', 'error');
    }
  };
        ...
        <AnimatePresence mode='popLayout'>
          {filteredChatters.map((chatter) => (
            <motion.div
              ...
              key={chatter.slug} className={`break-inside-avoid group relative ${chatter.hidden ? 'opacity-60' : ''}`}
            >
              {/* 悬浮管理按钮组 */}
              {canManage && (
                <div className="absolute top-5 left-5 z-20 flex gap-2 opacity-30 group-hover:opacity-100 transition-all duration-300 -translate-x-2 group-hover:translate-x-0">
                  <Link
                  ...
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      toggleHidden(chatter);
                    }}
                    className={`w-8 h-8 rounded-lg text-white flex items-center justify-center shadow-lg transition-colors ${chatter.hidden ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-slate-700 hover:bg-slate-900'}`}
                    title={chatter.hidden ? '恢复显示' : '暂时隐藏'}
                  >
                    {chatter.hidden ? <Eye size={14} /> : <EyeOff size={14} />}
                  </button>
                </div>
              )}
              ...
              >
                {canManage && chatter.hidden && (
                  <div className="absolute top-5 right-5 z-20 rounded-xl bg-slate-950/70 px-3 py-1 text-[10px] font-black text-white backdrop-blur-md">
                    已隐藏
                  </div>
                )}
                {chatter.cover && (
